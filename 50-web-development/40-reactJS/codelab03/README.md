...
Back to our **ProfileGallery** component. Declare a functional component called ProfileGallery. At the very start of our function, we are going to declare the hooks we will be working with. For now, we just want to include a **useState()** hook that will enable us to keep and change a list of pet objects. We will write this as follows:
```
...
We will implement this useEffect() hook as follows:

```
getPets().then((result) => setPets(result.data)).then(setState('defaultComponentState')));
}, [componentState]);
```
Implementing this into your app will break it. This is something you can fix yourself by checking the error and going back to the part about state in this codelab. Make the error disappear before you move on to the next part.
